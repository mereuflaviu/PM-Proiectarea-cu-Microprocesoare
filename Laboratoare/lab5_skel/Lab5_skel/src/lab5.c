...
void init_directory() {
	...
	FRESULT res = pf_opendir(&directory, "/music");
	int has_file = 0;
	if (res != FR_OK)
	{
		LCD_printAt(0x00, "error opening dir");
		return;
	}

	/* TODO3: find the first file whose name doesn't start with a `_`; use the `directory` and `file` structures declared above */
	while (pf_readdir(&directory, &file) == FR_OK && file.fname[0] != '\0')
	{
		// Check if file name doesn't start with underscore
		if (file.fname[0] != '_')
		{
			...
			has_file = 1;
			break; // Stop after finding the first valid audio file
		}
	}
	...
	{
		LCD_clear_top_line();
		LCD_printAt(0x00, "no files found");
	}
}
...
int main (void)
{
	...
	for(;;)
	{
		...
		for(;;)
		{
			...
			if ((PIND & (1 << PD6)) == 0)
			{
				snprintf(path, sizeof(path), "/music/%s", file.fname);
